class Database {
    ...
    async addNewReminder(reminderObject, context) {
        const reminderRes = await this.pool.query(`
            INSERT INTO schedule
            (reminder_object, context, owner) 
            VALUES ($1, $2, $3)
            RETURNING id
        `, [JSON.stringify(reminderObject), JSON.stringify(context), context.payload.comment.user.login])

        return reminderRes.rows[0].id
    }

    async createReminderGroup(groupName, groupValue) {
        await this.pool.query(`
    ...
    async getReminderGroupByName(groupName) {
        const groupRes = await this.pool.query(`
            SELECT group_value FROM reminder_groups 
            WHERE group_name = $1
        `, [groupName])

        return groupRes.rows.length > 0 ? groupRes.rows[0] : null
    ...
    async getReminders() {
        const remindersRes = await this.pool.query(`SELECT * FROM schedule`)
        return remindersRes.rows
    }

    async getReminderById(id) {
        const reminderRes = await this.pool.query(`SELECT * FROM schedule WHERE id=$1
        `, [id])
        return reminderRes.rows.length > 0 ? reminderRes.rows[0] : null
    }

    async getRemindersForUser(owner) {
        const remindersRes = await this.pool.query(`SELECT * FROM schedule WHERE owner = $1`, [owner])
        return remindersRes.rows
    }
}
