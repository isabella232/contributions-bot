...
async function processIssueComment({ context, commentReply, db }) {
    ...
    if (firstWord === "remind") {
        await processReminderRequest({ context, commentReply, db })
    } else {
...
async function sendContributorMerch(who, isCodeContribution, db, extraMerch) {
 ...
 async function processReminderRequest({ context, commentReply, db }) {
    const commentBody = context.payload.comment.body
    const reminderObject = parseReminder(commentBody.split(' ').slice(1).join(' '), context.payload.comment.created_at)
    ...
    schedule.scheduleJob(date, async () => await remind(reminderObject, reminderId, context, db))
}
...
async function remind(reminderObject, reminderId, context, db) {
    ...
    cr.reply(
        `Hey ${reminderObject.who === 'me' ? requestingUser : reminderObject.who}! ${reminderObject.who === 'me' ? 'You' : `@${reminderObject.who}`} asked me to remind you of the following: "${reminderObject.what}"`
    )
    await cr.send(true)
...
module.exports = { processIssueComment, processContribution, remind }
