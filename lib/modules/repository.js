const { AllContributorBotError, BranchNotFoundError, ResourceNotFoundError } = require('./errors')
...
class Repository {
    ...
    constructor(context) {
        ...
        this.repo = process.env.TARGET_REPO
        this.owner = process.env.TARGET_OWNER
    ...
    async createOrUpdateFiles({ filesByPath, branchName }) {
        const repository = this
        const createOrUpdateFilesMultiple = Object.entries(filesByPath).map(([filePath, { content, originalSha }]) => {
    ...
    async getPullRequestURL({ branchName }) {
        try {
            ...
            throw new AllContributorBotError(`A pull request is already open for the branch \`${branchName}\`.`)
        }
    }
    ...
    async createPullRequest({ title, body, branchName }) {
        ...
        try {
            ...
            if (pullRequestURL) {
                const splitURL = pullRequestURL.split('/')
                this.octokit.request('POST /repos/{owner}/{repo}/issues/{number}/labels', {
                    owner: this.owner,
                    repo: this.repo,
